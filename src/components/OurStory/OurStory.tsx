...
export default function OurStory() {
	...
	const updateStoryEntries = useStoryStore((state) => state.updateStoryEntries);

	useEffect(() => {
		if (storyEntries.size > 0) {
			return; // Story entries already loaded, no need to fetch again
		}
		getStoryEntries()
		...
		>
			...
			{Array.from(storyEntries.values()).map((entry) => (
				<StorySection
					key={entry.id}
					header={entry.header}
					text_body={String(entry.text_body)}
